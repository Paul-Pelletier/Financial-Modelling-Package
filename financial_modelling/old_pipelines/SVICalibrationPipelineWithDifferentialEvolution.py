...
from financial_modelling.modelling.SVIModel_Differential_Evolution import SVIDifferentialEvolutionModel

class SVICalibrationPipelineWithDE:
    ...
    def fit_model(self, preprocessed_data, model=None):
        ...
        """
        if model is None:
            model = SVIDifferentialEvolutionModel()
    ...
    def plot_fitted_models(self, results):
        ...
        for expiry, train_data, model_params in results:
            ...
            maturity = train_data["Residual_Maturity"].values[0]  # All rows share the same maturity
            ...
            total_variance = svi_formula(log_moneyness_range, a, b, rho, m, sigma)
            fitted_volatility = np.sqrt(np.maximum(total_variance / maturity, 0))  # Adjust for maturity
            ...
            plt.title(f"Fitted SVI Model for Expiry {expiry}")
...
if __name__ == "__main__":
    ...
    pipeline = SVICalibrationPipelineWithDE(DatabaseFetcher, IVPreprocessor)
    pipeline.run("D://")
